...
class AssessmentWorkflowTest extends TestCase
{
    ...
    private function assessment(): array
    {
        ...
        $assignment = $course->assignments()->create(['title' => 'Analysis', 'instructions' => 'Explain the evidence', 'due_at' => now()->subHour(), 'max_marks' => 20]);
    ...
    public function test_rubric_scoring_is_bounded_audited_and_locked_after_submission(): void
    {
        ...
        $this->actingAs($student)->get(route('assignments.show', $assignment))->assertOk()->assertSee('17.75')->assertSee('Moderation correction')->assertSee('Good evidence');
    }
    ...
    public function test_extensions_are_private_monotonic_and_update_lateness(): void
    {
        ...
        $this->actingAs($student)->post(route('assignments.submit', $assignment), ['body' => 'Late answer'])->assertRedirect();
        $this->assertTrue(Submission::firstOrFail()->is_late);
